...
export function useResume(storageKey = STORAGE_KEY) {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.styleConfig) setStyleConfig(parsed.styleConfig);
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      const fullData = { ...data, styleConfig, templateKey, documentType };
      localStorage.setItem(storageKey, JSON.stringify(fullData));
    }, 400);
    return () => {
      if (saveTimer.current) clearTimeout(saveTimer.current);
    };
  }, [data, styleConfig, templateKey, documentType, loaded, storageKey]);
  ...
  const updateProject = useCallback((id: string, updates: Partial<ProjectItem>) => {
    setData((prev) => ({
      ...prev,
      projects: prev.projects.map((e) => (e.id === id ? { ...e, ...updates } : e)),
    }));
  }, []);
  ...
  const updateSkillGroup = useCallback((id: string, updates: Partial<SkillGroup>) => {
    setData((prev) => ({
      ...prev,
      skills: prev.skills.map((e) => (e.id === id ? { ...e, ...updates } : e)),
    }));
  }, []);
  ...
  const updateCertification = useCallback((id: string, updates: Partial<CertificationItem>) => {
    setData((prev) => ({
      ...prev,
      certifications: prev.certifications.map((e) => (e.id === id ? { ...e, ...updates } : e)),
    }));
  }, []);
  ...
  const updateCustomEntry = useCallback((sectionId: string, entryId: string, updates: Partial<CustomSectionEntry>) => {
    setData((prev) => ({
      ...
      customSections: prev.customSections.map((s) =>
        s.id === sectionId
          ? { ...s, entries: s.entries.map((e) => (e.id === entryId ? { ...e, ...updates } : e)) }
          : s
      ),
    }));
  }, []);
  ...
  const addBullet = useCallback(
    (section: "experience" | "education" | "projects", itemId: string) => {
      setData((prev) => ({
        ...prev,
  ...
  const updateBullet = useCallback(
    (section: "experience" | "education" | "projects", itemId: string, bulletIdx: number, text: string) => {
      setData((prev) => ({
        ...prev,
        [section]: (prev[section] as Array<{ id: string; bullets: string[] }>).map((item) =>
          item.id === itemId
            ? { ...item, bullets: item.bullets.map((b, i) => (i === bulletIdx ? text : b)) }
            : item
        ),
      }));
    },
  ...
  const removeBullet = useCallback(
    (section: "experience" | "education" | "projects", itemId: string, bulletIdx: number) => {
      setData((prev) => ({
        ...prev,
        [section]: (prev[section] as Array<{ id: string; bullets: string[] }>).map((item) =>
          item.id === itemId
  ...
  }, [data, styleConfig, templateKey, documentType]);
