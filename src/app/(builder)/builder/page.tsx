...
export default function BuilderPage() {
  ...
  const handleImportFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (resume.importJSON(text)) {
        setShowImportModal(false);
        setImportText("");
        setImportError("");
      } else {
        setImportError("Invalid resume JSON file");
      }
    };
  ...
  const handleImportText = () => {
    if (resume.importJSON(importText)) {
      setShowImportModal(false);
      setImportText("");
      setImportError("");
    } else {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="lg:hidden flex border-b border-gray-200 bg-white print:hidden">
        <button
          className={`flex-1 py-3 text-sm font-medium text-center transition-colors relative ${
            mobileTab === "edit" ? "text-primary" : "text-gray-500 hover:text-gray-700"
          }`}
        ...
        <button
          className={`flex-1 py-3 text-sm font-medium text-center transition-colors relative ${
            mobileTab === "preview" ? "text-primary" : "text-gray-500 hover:text-gray-700"
          }`}
          ...
          <div className="p-5 sm:p-6">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-1.5">
                ...
                <button onClick={handlePrint} className="btn-secondary text-xs px-2.5 py-1.5">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            {showTips && (
              <div className="mb-5 p-4 rounded-xl bg-amber-50 border border-amber-200 animate-fade-in">
                ...
                <ul className="text-xs text-amber-700 space-y-1.5 ml-1">
                  ...
                  <li>• Keep formatting simple — avoid tables, columns, or graphics</li>
                  <li>• Use standard fonts and readable font sizes</li>
            ...
            <div className="flex flex-wrap gap-1.5 mb-5">
              ...
              <button onClick={() => setShowImportModal(true)} className="btn-ghost text-xs px-2.5 py-1.5" title="Import JSON">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              ...
              <button
                ...
                className="btn-ghost text-xs px-2.5 py-1.5 text-red-500 hover:text-red-700 hover:bg-red-50"
            ...
            <div className="space-y-5">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {templateOptions.map((t) => (
                    <button
                      key={t.key}
                      onClick={() => resume.setTemplateKey(t.key)}
              ...
              {/* ─── Summary ─── */}
              <section>
                <SectionToggle title="Professional Summary" onClick={() => toggleSection("summary")} open={isSectionOpen("summary")} />
                {isSectionOpen("summary") && (
                  <div className="animate-fade-in">
                    <textarea
                      ...
                      onChange={(e) => resume.updateSummary(e.target.value)}
                    />
                    <div className="flex justify-between mt-1.5">
                      <p className="text-xs text-gray-400">2-4 sentences recommended</p>
                      <p className={`text-xs ${resume.data.summary.length > 900 ? "text-amber-500" : "text-gray-400"}`}>
                        {resume.data.summary.length}/1000
                      </p>
                    </div>
                  </div>
                )}
              ...
              {/* ─── Experience ─── */}
              <section>
                <CollapsibleHeader title="Experience" count={resume.data.experience.length} onAdd={resume.addExperience} onClick={() => toggleSection("experience")} open={isSectionOpen("experience")} />
              ...
              {/* ─── Education ─── */}
              <section>
                <CollapsibleHeader title="Education" count={resume.data.education.length} onAdd={resume.addEducation} onClick={() => toggleSection("education")} open={isSectionOpen("education")} />
              ...
              {/* ─── Skills ─── */}
              <section>
                <CollapsibleHeader title="Skills" count={resume.data.skills.length} addLabel="Add Group" onAdd={resume.addSkillGroup} onClick={() => toggleSection("skills")} open={isSectionOpen("skills")} />
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {group.items.map((item, i) => (
                            <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium">
                              {item}
                              <button onClick={() => resume.removeSkillItem(group.id, i)} className="hover:text-red-500 ml-0.5">
                          ...
                          <input
                            ...
                            onKeyDown={(e) => {
                              if ((e.key === "Enter" || e.key === ",") && skillInput[group.id]?.trim()) {
                                e.preventDefault();
                                resume.addSkillItem(group.id, skillInput[group.id].trim());
                                setSkillInput((p) => ({ ...p, [group.id]: "" }));
                              }
                            }}
                          />
                          <button
                            onClick={() => {
                              if (skillInput[group.id]?.trim()) {
                                resume.addSkillItem(group.id, skillInput[group.id].trim());
                                setSkillInput((p) => ({ ...p, [group.id]: "" }));
                              }
                            }}
              ...
              {/* ─── Projects ─── */}
              <section>
                <CollapsibleHeader title="Projects" count={resume.data.projects.length} onAdd={resume.addProject} onClick={() => toggleSection("projects")} open={isSectionOpen("projects")} />
              ...
              {/* ─── Certifications ─── */}
              <section>
                <CollapsibleHeader title="Certifications" count={resume.data.certifications.length} onAdd={resume.addCertification} onClick={() => toggleSection("certifications")} open={isSectionOpen("certifications")} />
              ...
              {/* ─── Links ─── */}
              <section>
                <CollapsibleHeader title="Links" count={resume.data.links.length} onAdd={resume.addLink} onClick={() => toggleSection("links")} open={isSectionOpen("links")} />
                {isSectionOpen("links") && (
                  <div className="space-y-3 animate-fade-in">
              ...
              <section>
                <CollapsibleHeader
                  ...
                  count={resume.data.customSections.length}
                  addLabel="Add Section"
                  onAdd={resume.data.customSections.length < 3 ? resume.addCustomSection : undefined}
                  onClick={() => toggleSection("custom")}
                  open={isSectionOpen("custom")}
                />
                {isSectionOpen("custom") && (
                  <div className="space-y-3 animate-fade-in">
                    {resume.data.customSections.length === 0 && <EmptySection text="Add a custom section (e.g. Awards, Languages)" onClick={resume.addCustomSection} />}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {accentOptions.map((c) => (
                        <button
                          key={c.key}
                          onClick={() => resume.setStyleConfig({ ...resume.styleConfig, accentTone: c.key })}
                          className={`w-7 h-7 rounded-full ${c.class} transition-all ${
                            resume.styleConfig.accentTone === c.key ? "ring-2 ring-offset-2 ring-primary scale-110" : "hover:scale-110"
                          }`}
                          title={c.label}
                        />
                      ))}
        ...
        <div className={`flex-1 overflow-y-auto bg-gray-100/80 custom-scrollbar ${mobileTab !== "preview" ? "hidden lg:block" : ""}`}>
          <div className="p-4 sm:p-6 lg:p-8 min-h-full flex flex-col items-center">
            <div className="w-full max-w-[210mm] flex items-center justify-between mb-4 print:hidden">
              ...
              <button onClick={handlePrint} className="btn-ghost text-xs px-3 py-1.5">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.229 2.523a1.125 1.125 0 01-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0021 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 00-1.913-.247M6.34 18H5.25A2.25 2.25 0 013 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 011.913-.247m10.5 0a48.536 48.536 0 00-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18.75 12h.008v.008h-.008V12zm-3 0h.008v.008H15.75V12z" />
                </svg>
                Print / PDF
      ...
      <div className="lg:hidden border-t border-gray-200 bg-white px-4 py-3 flex gap-2 print:hidden">
        <button onClick={handlePrint} className="btn-secondary flex-1 text-xs py-2.5 justify-center">
        ...
        <button className="btn-primary flex-1 text-xs py-2.5 justify-center">
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm print:hidden">
          <div className="card p-6 max-w-lg mx-4 w-full animate-fade-in">
            <h3 className="text-lg font-bold text-gray-900 mb-1">Import Resume</h3>
            <p className="text-sm text-gray-500 mb-4">Paste your resume JSON or upload a file exported from ResumeForge.</p>
            <textarea className="input-modern resize-none mb-3 font-mono text-xs" rows={8} placeholder="Paste JSON here..." value={importText} onChange={(e) => { setImportText(e.target.value); setImportError(""); }} />
            {importError && <p className="text-xs text-red-500 mb-3">{importError}</p>}
            <div className="flex gap-3">
              <button onClick={() => { setShowImportModal(false); setImportText(""); setImportError(""); }} className="btn-secondary flex-1">Cancel</button>
              <button onClick={() => fileInputRef.current?.click()} className="btn-secondary flex-1">Upload File</button>
              <button onClick={handleImportText} className="btn-primary flex-1" disabled={!importText.trim()}>Import</button>
            </div>
    ...
    <div className="space-y-1.5">
      ...
      {bullets.map((bullet, i) => (
        <div key={i} className="flex gap-2 items-start">
          ...
          <button onClick={() => onRemove(i)} className="p-1.5 rounded hover:bg-red-100 text-gray-400 hover:text-red-500 mt-1">
            <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
      ))}
